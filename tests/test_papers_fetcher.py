...
def test_fetch_all_papers_sorts_by_published_at() -> None:
    papers = [
        _paper("openalex:W1", datetime(2020, 1, 1, tzinfo=timezone.utc)),
        _paper("openalex:W2", datetime(2026, 6, 1, tzinfo=timezone.utc)),
    ]

    with patch("src.papers.fetcher.OpenAlexFetcher") as MockOpenAlex:
        MockOpenAlex.return_value = _fetcher_returning(papers)

        config = PapersConfig(enabled=True)
        result = asyncio.run(fetch_all_papers(config, client=None))

    assert [p.id for p in result] == ["openalex:W2", "openalex:W1"]
