...
# 只配置本模块 logger，不调用 logging.basicConfig：basicConfig 会把全局
# root 级别抬到 INFO，导致 httpx 等第三方库的每次 HTTP 请求日志刷屏，
# 冲散 rich 进度条。这里给本模块一个独立 handler，登录信息依然可见。
...
def configure(data_dir: str = "data/wxmp") -> None:
    """Point QR/lock files at *data_dir* (called from ``we_mp_rss.init``)."""
    global _data_dir
    _data_dir = data_dir
...
class WeChatAPI:
    ...
    def __init__(self):
        ...
        self.qr_code_path = os.path.join(_data_dir, "wx_qrcode.png")
        self.wx_login_url=f"{self.qr_code_path}"
        self.lock_file_path=os.path.join(_data_dir, "lock.lock")
        # 线程安全
        self._lock = Lock()
    ...
    async def switch_account(self, username: str = ""):
        """切换微信公众号账号（异步）—— Playwright 登录模块未内置，不支持。"""
        self.login_with_token()
        raise NotImplementedError(
            "switch_account 未内置（driver.wx Playwright 登录模块未打包）"
        )
    def _redirect(self):
        ...
        try:
            with self._lock:
                ...
                if has_fail_indicator:
                    print_warning("检测到登录失败标识，Token已失效，请重新扫码登录 (horizon-wxmp login)")
                    return False
    ...
    def GetCode(self,CallBack=None,Notice=None):
        ...
        import threading
        self.thread = threading.Thread(target=self.get_qr_code, args=(CallBack, Notice), daemon=True)
...
def _get_wechat_api() -> WeChatAPI:
    global _wechat_api_instance
    if _wechat_api_instance is None:
        _wechat_api_instance = WeChatAPI()
    return _wechat_api_instance


def __getattr__(name: str):
    """PEP 562 — expose a lazy module-level ``WeChat_api`` singleton so
    importing this module has no side effects (no data-dir creation)."""
    if name == "WeChat_api":
        return _get_wechat_api()
