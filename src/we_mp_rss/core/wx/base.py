...
import re
import time
from ..models_status import DATA_STATUS
from .cfg import cfg
...
class WxGather:
    ...
    def Model(self,type=None):
        ...
        print(f"采集模式:{type}")
        if type=="app":
            from .model.app import MpsAppMsg
            wx=MpsAppMsg()
        elif type=="web":
            from .model.web import MpsWeb
            wx=MpsWeb()
        else:
            from .model.api import MpsApi
            wx=MpsApi()
        return wx
    ...
    def Error(self,error:str,code=None):
        ...
        if code=="Invalid Session":
            setStatus(False)
            print_error("公众号平台登录失效,请重新扫码登录 (horizon-wxmp login)")
    ...
    # 更新公众号更新状态（内置版无 DB，no-op）
    def update_mps(self, mp_id: str, mp=None):
        """更新公众号同步状态和时间信息（内置版：无数据库，no-op）。"""
        pass