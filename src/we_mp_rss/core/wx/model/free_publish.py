"""Free-publish multi-endpoint fallback (we-mp-rss ``core/wx/model/free_publish.py``).

When the primary ``appmsgpublish`` list endpoint is rate-limited or blocked
(ret != 0), ``MpsFreePublish`` probes several WeChat MP list endpoints in turn
and uses the first one that returns valid data.  Bundled-version adjustments:
relative imports, and ``print_success`` added to the import list (upstream
    ...
    def get_Articles(
        ...
        for endpoint_config in self.ENDPOINTS:
            ...
            try:
                ...
                if ret != 0:
                    print_warning(
                        f"{endpoint_name}: 返回错误 ret={ret}, "
                        f"err_msg={msg.get('base_resp', {}).get('err_msg', '')}"
                    )
                    continue
