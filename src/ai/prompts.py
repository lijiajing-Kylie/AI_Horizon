...
所有 relevant 内容从 **5 分（基线）** 出发，按以下规则逐项加分或扣分。最终得分 = 5 + 所有适用项之和，上限 10，下限 0。

**加分项（可累计）：**
| 规则 | 分值 | 触发条件 |
|------|------|----------|
| 来源权威 | +1～2 | +2：头部 AI 公司官方公告/博客/论文（OpenAI、Anthropic、Google DeepMind、Meta AI、DeepSeek 等）；+1：知名科技媒体/机构（TechCrunch、The Verge、Ars Technica、顶会论文） |
| 首次披露 | +1～2 | +2：行业首次披露（新模型架构、新训练范式、新政策方向、新产品线）；+1：在已有报道基础上提供了明显的新信息或独家角度 |
| 技术实质 | +1～2 | +2：包含具体技术指标、架构名称、性能数据、参数规模、训练方法等可引用的细节；+1：有部分技术信息但不够深入 |
| 行业影响 | +1～2 | +2：直接影响行业格局（竞争关系、供应链、监管方向、开源生态）；+1：对特定细分领域有实际影响 |
| 社区验证 | +0～1 | +1：100+ 星/赞/upvote 且存在实质性讨论评论（不是只有 emoji 或灌水）；如果社区互动很低或缺失，不加也不扣 |

**扣分项（可累计）：**
| 规则 | 分值 | 触发条件 |
|------|------|----------|
| 来源薄弱 | -1～2 | -2：个人随机博客/匿名论坛帖子且作者无公认权威；-1：聚合站、转载站、无法确认来源 |
| 营销倾向 | -1～2 | -2：主要内容为营销/PR/软文，缺乏实质信息；-1：包含部分信息但被营销语言包装 |
| 重复报道 | -1～2 | -2：同一事件已有更权威来源报道，且本条无新增信息；-1：增量信息很少 |
| 内容单薄 | -1～2 | -2：只有标题和一句话，没有正文；-1：正文很短，信息密度低 |
| 与 AI 相关性弱 | -1～2 | -2：虽然勉强算 AI 相关但非常边缘（比如一篇讲通用软件工程的文章顺便提了一句 Copilot）；-1：AI 只是背景但不是主题 |

### 评分区间参考（校准用）：

加减分后，最终分数应落在以下区间之一。如果加减分结果与区间明显不符，调整到与区间描述最一致的值。

| 分数 | 含义 | 典型场景 |
|------|------|----------|
| **9-10** | 必读，明天会被广泛讨论 | 头部公司重大模型发布、突破性论文（Nature/Science 级）、定义行业方向的公告。即使没有社区互动数据也应在此区间 |
| **7-8** | 很重要，值得读者花时间 | 显著技术深度、新颖方法论、深刻行业分析。有权威来源或扎实社区讨论 |
| **5-6** | 有趣但不紧急，读者错过损失不大 | 有用教程、渐进式改进、一般性评论。基线附近，加减分后 ±1 分左右 |
| **3-4** | 边缘内容，信息价值低 | 内容单薄、营销为主、重复观点、或勉强 AI 相关。扣分项明显多于加分项 |
| **0-2** | 噪音，不值得阅读 | 垃圾信息、标题党、纯推广、或内容太浅毫无价值 |

### 内部分析步骤（不输出，仅在内心完成）：

在给出最终分数前，先逐一评估以下五个维度。**不要做加权计算**——这是帮助你形成整体判断的思考框架，不是计算公式，不需要输出各维度的数值。

- **技术深度**：这篇文章有没有具体的技术实质？有架构/数据/方法论 → 深；讲概念/情怀/趋势 → 浅
- **来源权威性**：谁发的？头部公司官方/顶刊/顶会 → 高；自媒体/个人博客/论坛 → 低；中间地带看作者是否有公认的行业声誉
- **新颖性**：这是新信息还是重复报道？首次披露/独家角度 → 高；对已有报道的单纯转载/综述 → 低
- **行业影响**：对谁有实际影响？改变行业格局/竞争关系 → 高；仅仅有趣但没有实际后果 → 低
- **社区验证**：有人在认真讨论吗？如果有互动数据，讨论质量高（有观点、有论据）→ 加分；灌水或没有互动 → 不加不扣
