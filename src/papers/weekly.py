...
class _DetailSegment(NamedTuple):
    name: str              # "overview" / "method" / "evaluation"
    system: str
    user_template: str
    fields: tuple[str, ...]  # 段内普通字符串字段（keywords/innovation_level 单独处理）
...
def _build_segment_user(
    ...
    """Fill one segment's user template.

    ``prior_summary`` carries the fields already generated by earlier segments
    as a consistency anchor (segment 1 has none, so it's omitted). The scoring
    reason is stripped of any old ``[enrich ...]`` marker so it can't leak into
    the prompt.
    """
    reason = (paper.ai_reason or "").split("[enrich")[0].strip()
    kwargs = dict(
        title=paper.title,
        authors=", ".join(paper.authors[:10]),
        categories=", ".join(paper.categories),
        ...
        prior_summary=json.dumps(prior_summary, ensure_ascii=False) if prior_summary else "（暂无）",
    )
...
async def _enrich_paper(
    ...
    # in 3 segments (overview / method / evaluation). Each segment emits 2-4
    # fields in one request; a failed or unparseable segment only drops its own
    ...
    for seg in _DETAIL_SEGMENTS:
        ...
        # keywords 与 innovation_level 由段 3 产出；前两段结果里不存在这两个 key。
    ...
    if summary:
        ...
        if failed_segments:
            _mark_enrich_failure(paper, "detail", f"partial: {','.join(failed_segments)}")
    elif failed_segments:
        _mark_enrich_failure(paper, "detail", f"segments failed: {','.join(failed_segments)}")
    else:
        _mark_enrich_failure(paper, "detail", "empty response")
