...
(overview / method / evaluation), each returning 2-4 fields. Segmented
...
# ---------------------------------------------------------------------------
#
# The full 12-field interpretation is generated in 3 requests. Each segment
# gets the shared base (role + general requirements + self-check) followed by
# its own field list. Keeping the base as one string avoids maintaining the
# general requirements three times; the segment-specific tails list only the
# fields that request is allowed to emit.

_ARXIV_DETAIL_BASE = """你是一名 AI 领域技术编辑，负责将学术论文转化为易理解、有洞察力的论文解读。
...
整篇解读分三部分生成：
- 概述：one_sentence_summary / why_it_matters / background / previous_problem
- 方法：core_idea / how_it_works / technical_details
- 评估：experimental_evidence / real_world_impact / limitations / innovation_level / keywords

每次请求只输出本段要求的字段，不得输出其他字段。
...
1. 是否只包含本段要求的字段？
2. 一个不了解该领域的人是否能理解核心贡献？
3. 是否回答了"为什么重要"？
4. 是否避免直接复制论文摘要？
5. 是否区分创新和工程优化？
6. 是否减少无必要的专业术语？
7. 是否与已给出的解读片段在术语和结论上保持一致？
...
本轮只生成以下 4 个字段（概述）。仅返回合法 JSON，不要 markdown：
{{
...
**已生成的解读片段（仅作上下文保持一致，不要重复输出这些字段）：**
{prior_summary}

本轮只生成以下 3 个字段（方法）。仅返回合法 JSON，不要 markdown：
{{
  "core_idea": "核心创新...",
  "how_it_works": "如何实现...",
